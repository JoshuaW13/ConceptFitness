...
function CatalogueDrawerContent({ plannedExercises, setPlannedExercises, tags, setTags, programName, setProgramName, setNumExercises, setSearchText, setSearchState }) {
  ...
  return (
    <div className="p-3 h-full w-full flex-col bg-[#8E8D8A]">
      <div className="flex flex-row justify-between m-2">
        <input type="text" onChange={handleNameInputChanged} placeholder="Edit Program Name Here" value={programName} className="w-[80%] h-8 text-black bg-[#EAE7DC] pl-2 text-left text-lg rounded-md">
        </input>
        <button className='w-[11%] bg-[#EAE7DC]' onClick={navigatePrograms}>
      ...
      <div className="flex flex-row justify-between m-2">
        <p className='text-[#EAE7DC] font-semibold mr-4'>Tags</p>
      ...
      >
        <SortableContext items={plannedExercises.map((exercise) => exercise.id)} strategy={verticalListSortingStrategy}>
          <div className='w-[93%] flex-grow h-[78%] bg-[#EAE7DC] flex flex-col rounded-lg gap-2 overflow-y-auto m-3 scrollbar-hidden'>
